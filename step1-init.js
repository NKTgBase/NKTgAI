...
export function setPipelineState(newState) {
    ...
    if (badge) {
        const label = window._nktgLanguage ? window._nktgLanguage.t('pipeline.label') : 'PIPELINE';
        badge.innerText = `${label}: ${newState}`;
...
onDOMReady(() => {
    ...
    if (sendArrow) {
        sendArrow.addEventListener('click', async function() {
            ...
            this.disabled = true;
            this.innerHTML = '<span class="nktg-spinner"></span>';
            try {
